...
- Manual-testing uncited LLM refusal fix: after Context Validation passed for `Can you tell me more about Ho Chi Minh?` against workspace `7`/repository `8`, the model returned the uncited self-refusal `There is not enough repository data to answer accurately.`. This is a second, independent gap from the earlier citation-format mismatch: the issue was not a near-miss citation syntax, but that the workflow did not recognize an uncited LLM self-decline as a safe refusal outcome. Phase 3 now converts any non-refused answer with no parseable citations into the standard refusal response, clears client-facing LLM metadata for that refusal shape, keeps citations empty, and logs the raw uncited model output server-side for future prompt/retrieval diagnostics.
...
- Manual-testing embedding provider leakage fix: invalid-key testing found the same provider-error leakage class on the Phase 2 custom embedding path used by retrieval. `OpenRouterEmbeddingClient` returned `Embedding Service Unavailable` with raw provider details such as the provider name and `401 Unauthorized`, while the earlier post-review fix only covered Spring AI chat generation. Phase 3 now translates embedding-provider failures through the same safe `503 AI Provider Unavailable` response (`The assistant is temporarily unavailable, please try again.`), logs the raw embedding exception server-side, and regression tests cover both the embedding client and `/chat` error envelope.
...
- Embedding provider failures are now sanitized consistently with chat provider failures after manual invalid-key testing exposed raw embedding HTTP/provider details in the client-facing 503 body.
